...
export default function App() {
  ...
  // Title block (headline + subtext + CTA) — independently draggable AND rotatable
  // Default positions keep them apart out of the box — social proof badge
  // sits near the top, title block (headline+subtext+CTA) is vertically
  // centered below it. Both are still fully draggable from here.
  const [titleTransform, setTitleTransform]           = useState({ x: 0, y: 8, rotation: 0 });
  // Social proof badge — independently draggable (no rotation)
  ...
  const resetApp = () => {
    ...
    setTitleTransform({ x: 0, y: 8, rotation: 0 }); setProofTransform({ x: 0, y: -30 });
          ...
          <motion.div initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }}
            ...
            <div className="order-1 lg:order-2 lg:col-span-7 space-y-6">
              <div className="bg-white p-6 sm:p-10 rounded-[3.5rem] border border-slate-200 shadow-2xl shadow-slate-200/50 flex flex-col">
                {/* Sticky header + canvas + size slider — stays pinned near the
                    top of the screen while you scroll to variant buttons,
                    Pinterest link, etc. below, so the live preview never
                    scrolls out of view mid-edit. */}
                <div className="sticky top-20 z-10 bg-white pb-2">
                ...
                <div ref={previewRef}
                  ...
                  {isGeneratingImage ? (
                    ...
                    <>
                      ...
                      {/* Draggable + rotatable title block (headline, subtext, CTA) */}
                      <div className="absolute inset-0 p-10 flex flex-col justify-center items-center pointer-events-none"
                        style={{ transform: `translate(${titleTransform.x}%, ${titleTransform.y}%) rotate(${titleTransform.rotation}deg) scale(${overlayScale})` }}>
                      ...
                      {/* Draggable social-proof badge — independent from the title block */}
                      <div className="absolute inset-0 flex items-center justify-center pointer-events-none"
                        style={{ transform: `translate(${proofTransform.x}%, ${proofTransform.y}%) scale(${overlayScale})` }}>
                        <div className="pointer-events-auto cursor-move select-none active:scale-95 transition-transform"
                ...
                {/* ── End sticky header+canvas+slider wrapper ── */}
