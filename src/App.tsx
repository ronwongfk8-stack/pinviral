...
export default function App() {
  ...
  // Title block (headline + subtext) — independently draggable, rotatable, resizable
  // Default positions keep them apart out of the box — social proof badge
  // sits near the top, title block stays vertically centered.
  const [titleTransform, setTitleTransform]           = useState({ x: 0, y: 0, rotation: 0 });
  const [titleScale, setTitleScale]                   = useState(1);
  // Social proof badge — independently draggable + resizable (no rotation)
  const [proofTransform, setProofTransform]           = useState({ x: 0, y: -30 });
  const [proofScale, setProofScale]                   = useState(1);
  const [activeDrag, setActiveDrag]                   = useState<null | "title" | "proof">(null);
  const [activeRotate, setActiveRotate]               = useState(false);
  const [activeResize, setActiveResize]               = useState<null | "title" | "proof">(null);
  ...
  const resetApp = () => {
    ...
    setTitleTransform({ x: 0, y: 0, rotation: 0 }); setProofTransform({ x: 0, y: -30 });
    setTitleScale(1); setProofScale(1); setEditingField(null);
...
No generic CTAs. Focus on social proof and value proposition.` });
  ...
  // ── Drag handlers — title block and social-proof badge move independently ───
  const handleDragStart = (target: "title" | "proof") => (e: React.MouseEvent | React.TouchEvent) => {
    e.stopPropagation();
  ...
  // ── Rotate handler — title block only, drag the small handle above it ───────
  const handleRotateStart = (e: React.MouseEvent | React.TouchEvent) => {
    e.stopPropagation();
    setActiveRotate(true);
  };
  const handleRotateEnd = () => setActiveRotate(false);
  const handleRotateMove = (e: React.MouseEvent | React.TouchEvent) => {
    if (!activeRotate || !titleRef.current) return;
    ...
    setTitleTransform(t => ({ ...t, rotation: angleDeg }));
  };
  // ── Resize handlers — drag the corner handle to scale each element,
  // independent of position/rotation ──────────────────────────────────────────
  const handleResizeStart = (target: "title" | "proof") => (e: React.MouseEvent | React.TouchEvent) => {
    e.stopPropagation();
    const ref = target === "title" ? titleRef.current : proofRef.current;
    if (!ref) return;
    ...
    resizeStartRef.current = {
      ...
      scale: target === "title" ? titleScale : proofScale,
    };
  ...
  const handleResizeMove = (e: React.MouseEvent | React.TouchEvent) => {
    ...
    const ref = activeResize === "title" ? titleRef.current : proofRef.current;
    if (!ref) return;
    const rect = ref.getBoundingClientRect();
    const cx = "touches" in e ? e.touches[0].clientX : e.clientX;
    const cy = "touches" in e ? e.touches[0].clientY : e.clientY;
                ...
                <div ref={previewRef}
                  ...
                  {isGeneratingImage ? (
                    ...
                    <>
                      ...
                      {/* Draggable + rotatable + resizable title block (headline, subtext).
                          Click either line of text to edit it in place — no side panel. */}
                      <div className="absolute inset-0 p-10 flex flex-col justify-center items-center pointer-events-none"
                        ...
                        <div ref={titleRef} className="relative w-full pointer-events-auto cursor-move select-none active:scale-95 transition-transform"
                          ...
                          {/* Rotate handle — drag this in a circle to rotate the title block */}
                          <button type="button"
                            ...
                            onMouseDown={handleRotateStart} onTouchStart={handleRotateStart}
                            title="Drag to rotate">
                            <RotateCw size={12} className="text-slate-500"/>
                          </button>
                          {/* Resize handle — drag to scale the title block */}
                          <button type="button"
                          ...
                          <div className="space-y-4 text-center">
                            <motion.div key={editableHeadline} initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} className="px-6">
                              {editingField === "headline" ? (
                                <input autoFocus value={editableHeadline}
                                  ...
                                  className="w-full bg-transparent text-white font-black text-3xl sm:text-5xl uppercase italic tracking-tighter leading-[0.8] text-center outline-none border-b-2 border-white/50 pb-1 [text-shadow:_0_10px_40px_rgb(0_0_0_/_95%)]"/>
                              ) : (
                                <h2 onClick={startEditing("headline")}
                                  className="text-white font-black text-3xl sm:text-5xl uppercase italic tracking-tighter leading-[0.8] [text-shadow:_0_10px_40px_rgb(0_0_0_/_95%),_0_4px_10px_rgb(0_0_0_/_60%)] mb-3 cursor-text">
                            ...
                            {editingField === "subtext" ? (
                              ...
                              <p onClick={startEditing("subtext")}
                                className="text-white text-[10px] sm:text-[11px] font-black leading-tight px-12 [text-shadow:_0_4px_12px_rgb(0_0_0_/_95%)] uppercase tracking-[0.2em] opacity-90 italic cursor-text">
                                {editableSubtext}
                              </p>
                            )}
                          </div>
                      ...
                      {/* Draggable + resizable social-proof badge — independent from the title block */}
                      <div className="absolute inset-0 flex items-center justify-center pointer-events-none"
                        ...
                        <div ref={proofRef} className="relative pointer-events-auto cursor-move select-none active:scale-95 transition-transform"
                          ...
                          {/* Resize handle — drag to scale the badge */}
                          <button type="button"
